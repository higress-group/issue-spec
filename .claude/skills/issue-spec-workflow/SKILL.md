...
- On self-hosted profiles, the real review agent runs `issue-spec --profile <self-hosted-profile> review sync --repo higress-group/issue-spec --implement <issue> --revision <revision> --id REVIEW-001 --agent <review-agent> --json`. A successful sync persists and reloads provider facts, then upserts one stable done REVIEW with an exact-current completion stamp even when there are zero findings. Never fabricate a finding or hand-author the completion stamp.
