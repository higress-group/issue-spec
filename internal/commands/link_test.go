...
type linkStrictBackend struct {
	...
	suffixAfterWrite       bool
	suffixOnOwnerList      int
	ownerLists             int
}

const linkTranslationDivider = "<!--This is a translation content dividing line, the content below is generated by machine, please do not modify the content below-->"

func linkBotSuffix(body string) string {
	return body + "\n\n" + linkTranslationDivider + "\n\n" + strings.ToUpper(body)
}
...
func (b *linkStrictBackend) ListIssueComments(_ context.Context, _ string, issue int) ([]github.Comment, error) {
	if issue == 3 {
		...
		if b.suffixOnOwnerList > 0 && b.ownerLists == b.suffixOnOwnerList {
			b.comments[3][0].Body = linkBotSuffix(b.comments[3][0].Body)
		}
	}
...
func (b *linkStrictBackend) update(id int64, body string) (github.Comment, error) {
	for issue, comments := range b.comments {
		for index := range comments {
			...
			if b.suffixAfterWrite {
				body = linkBotSuffix(body)
			}
