...
type Config struct {
	...
	Context      WorkflowContext     `json:"context,omitempty" yaml:"context"`
	...
	DurableSpecs *DurableSpecsConfig `json:"durable_specs,omitempty" yaml:"durable_specs"`
	HTMLReview   *HTMLReviewConfig   `json:"html_review,omitempty" yaml:"html_review"`
}

type DurableSpecsConfig struct {
	Mode durable.Mode `json:"mode" yaml:"mode"`
}

type HTMLReviewConfig struct {
	Enabled bool `json:"enabled" yaml:"enabled"`
}

// WorkflowContext accepts both the mapping form generated by issue-spec and
// the long-form scalar context used by established OpenSpec repositories.
type WorkflowContext map[string]any

func (c *WorkflowContext) UnmarshalYAML(value *yaml.Node) error {
	if value == nil {
		*c = nil
		return nil
	}
...
// ExternalCodeConfig is repository-owned provider selection only. An
...
func (p Plan) DurableSpecsMode() durable.Mode {
	if p.Config.DurableSpecs == nil {
		return durable.ModeNone
	}
	mode, err := durable.NormalizeMode(p.Config.DurableSpecs.Mode)
	if err != nil {
		return durable.ModeNone
	}
	return mode
...
func (p Plan) HTMLReviewEnabled() bool {
	if p.Config.HTMLReview == nil {
		return true
	}
...
func readConfig(path string) (Config, error) {
	...
	if err := validateDurableSpecsConfig(data, cfg.DurableSpecs); err != nil {
		return Config{}, err
	}
	if err := validateHTMLReviewConfig(data, cfg.HTMLReview); err != nil {
		return Config{}, err
	}
	if cfg.DurableSpecs != nil {
		mode, _ := durable.NormalizeMode(cfg.DurableSpecs.Mode)
		cfg.DurableSpecs.Mode = mode
	}
	return cfg, nil
}

func validateHTMLReviewConfig(data []byte, config *HTMLReviewConfig) error {
	var document yaml.Node
	if err := yaml.Unmarshal(data, &document); err != nil {
		return err
	}
	node := mappingValue(&document, "html_review")
	if node == nil {
		return nil
	}
	if node.Kind != yaml.MappingNode || config == nil {
		return errors.New("html_review must be a mapping")
	}
	if unknown := unknownMappingKeys(node, map[string]bool{"enabled": true}); len(unknown) > 0 {
		return fmt.Errorf("html_review contains unsupported fields: %s", strings.Join(unknown, ", "))
	}
	enabled := mappingValue(node, "enabled")
	if enabled == nil {
		return errors.New("html_review.enabled is required when html_review is configured")
	}
	if enabled.Kind != yaml.ScalarNode || enabled.Tag != "!!bool" {
		return errors.New("html_review.enabled must be a boolean")
	}
	return nil
}

func validateDurableSpecsConfig(data []byte, config *DurableSpecsConfig) error {
	var document yaml.Node
	if err := yaml.Unmarshal(data, &document); err != nil {
		return err
	}
	node := mappingValue(&document, "durable_specs")
	if node == nil {
		return nil
	}
	if node.Kind != yaml.MappingNode || config == nil {
		return errors.New("durable_specs must be a mapping")
	}
	if unknown := unknownMappingKeys(node, map[string]bool{"mode": true}); len(unknown) > 0 {
		return fmt.Errorf("durable_specs contains unsupported fields: %s", strings.Join(unknown, ", "))
	}
	if mappingValue(node, "mode") == nil {
		return errors.New("durable_specs.mode is required when durable_specs is configured")
	}
	mode, err := durable.NormalizeMode(config.Mode)
	if err != nil {
		return err
	}
	if mode == durable.ModeNone && strings.TrimSpace(string(config.Mode)) == "" {
		return errors.New("durable_specs.mode is required when durable_specs is configured")
	}
	return nil
}
...
func validateExternalCodeConfig(data []byte, config *ExternalCodeConfig) error {
	...
	if unknown := unknownMappingKeys(external, map[string]bool{"provider_key": true}); len(unknown) > 0 {
...
func decodeArtifact(node *yaml.Node, id string) (Artifact, error) {
	...
	artifact := Artifact{
		...
		Instructions:  strings.TrimSpace(firstNonEmpty(raw.Instructions, raw.Instruction)),
		ApplyTracks:   strings.TrimSpace(raw.Apply.Tracks),
		UnknownFields: unknownArtifactFields(node),
	}
	artifact.Dependencies = stringList(firstNonEmptyNode(raw.Dependencies, raw.DependsOn, raw.DependsOnAlt, raw.Requires))
	return artifact, nil
}
