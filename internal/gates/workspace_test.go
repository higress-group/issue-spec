...
func TestWorkspaceEvidenceIndependentChangeBearingDoesNotRequireLease(t *testing.T) {
	independent := workspaceGateProcess(t, model.ProcessExecutionChangeBearing, false, "")
	...
	if err != nil || hasBlockingWorkspaceDiagnostic(report.Diagnostics) || workspaceHasCode(report.Diagnostics, CodeProcessWorkspaceRequired) {
		t.Fatalf("independent diagnostics=%+v err=%v", report.Diagnostics, err)
	}
}
