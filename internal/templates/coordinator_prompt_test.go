...
func TestCoordinatorPromptConstructsNewCommandContract(t *testing.T) {
	...
	for _, want := range []string{
		...
		"plan the PROCESS DAG first. The coordinator MAY implement a non-trivial coding node inline or delegate it to a worker sub-agent",
		"inline coding is allowed for any node because independent review is mandatory",
		...
		"The review MUST be performed by a different agent than the one that authored the code under review",
