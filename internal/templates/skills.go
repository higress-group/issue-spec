...
- During Implement planning, make each PROCESS own one independently verifiable Design invariant and its major entry points while explicitly balancing end-to-end invariant cohesion against a bounded role-agent context and working set. Split only across a stable interface when every resulting node has independent acceptance criteria and can be reviewed correctly in isolation. File overlap, disjoint ownership, parallelism, command entry points, and individual findings are secondary scheduling inputs, never semantic boundaries. Neither the CLI nor generated guidance may infer semantic decomposition from paths, prose, token/file/line formulas, or CODEX_THREAD_ID.
- If neither a bounded cohesive PROCESS nor a defensible stable-interface split can be established, stop the Implement transition as blocked, present concrete competing boundary options and their acceptance consequences, and request human direction before dispatch. For a high-risk invariant, place an independent bounded review immediately after its cohesive change-bearing PROCESS. Repairs extend or replace the invariant-owning PROCESS; create a separate repair node only for a genuinely independent invariant.
...
1. Plan the PROCESS DAG before dispatch: read the authoritative Design and each active TASK's ### Execution Planning metadata, then record one independently verifiable Design invariant and its major entry points per PROCESS. Explicitly balance end-to-end invariant cohesion against a bounded role-agent context and working set. Split only across a stable interface with independent acceptance criteria and isolated reviewability; copy the invariant exactly into design_context.invariant. Treat file overlap, parallelism, command entry points, and individual findings as secondary scheduling inputs, not decomposition authority.
...
4. If neither a bounded cohesive PROCESS nor a defensible split can be established, stop the Implement transition as blocked, publish concrete competing boundary options and their acceptance consequences, and request human direction; never infer the split from paths, prose, token/file/line formulas, or CODEX_THREAD_ID. Otherwise select ready PROCESS nodes whose dependencies are done. Dispatch every agent-executed change-bearing node and review node to its assigned real worker or review agent. Include each dispatched agent's assigned subagent/session id and require it to pass that id with --agent-session on supported issue-spec writer commands. One real worker MAY execute multiple compatible serial change-bearing or code-repair nodes; a fresh worker is not required for every PROCESS, but each node retains distinct status, dependencies, workspace lifecycle, evidence, and handoff. Only after the semantic split criteria pass may disjoint write ownership enable parallel execution.
...
8. Dispatch an independent review PROCESS for every active SPEC that has a valid change-bearing carrier once reviewable implementation code exists on the selected code provider. Place that bounded review immediately after a cohesive high-risk invariant before downstream work depends on it. On GitHub, do not wait for PR rationale, which is added only after review/fix convergence. Each review node is owned by an agent that did not author the code under review. For each distinct change-bearing author Agent, the coordinator SHOULD provide at least one independent review assignment covering that author's PROCESS outputs and affected SPECs; one reviewer MAY cover multiple authors, and this does not add a 1:1 final gate. Run review nodes in parallel only when their review scopes are independent. A repair extends or replaces the invariant-owning PROCESS; create a separate repair PROCESS only when a finding exposes a genuinely independent invariant, then converge all fixes before final evidence.
...
10. Follow issue-spec-workflow for backend-appropriate implementation-change closure, including the managed issue-closing block's final-write and verification rules when that route uses it.
11. Add final rationale only after review/fix convergence and only for change-bearing PROCESS nodes. Follow issue-spec-workflow for the backend-appropriate rationale command. Each owning worker authors its own rationale under that worker's --agent and --agent-session. The coordinator MUST NOT create worker rationale or relabel its identity on the worker's behalf. Every class still requires TASK, implementation-change, and active SPEC traceability.
12. Mark PROCESS comments done only after implementation/review work and focused verification evidence exist.

## Coordinator DAG Execution

1. Derive the DAG from the authoritative Design plus TASK ### Execution Planning metadata. Record one independently verifiable Design invariant and its major entry points per PROCESS, explicitly balance invariant cohesion against the assigned role's bounded context and working set, and default to serial PROCESS chains under one parent TASK.
...
5. Split only across a stable interface when every resulting node has independent acceptance criteria and isolated reviewability. File overlap, parallelism, command entry points, and findings are secondary. If no bounded cohesive node or defensible split exists, stop the Implement transition as blocked, present concrete competing boundary options and acceptance consequences, and request human direction. Never infer semantic decomposition from paths, prose, token/file/line formulas, or CODEX_THREAD_ID. Only after those semantic criteria pass may disjoint write ownership enable parallel execution. Give each worker an assigned id to pass via --agent-session.
...
8. Every active SPEC with a valid change-bearing carrier MUST be reviewed by a review agent that is not the author of the code under review; spawn or assign it after the code is reviewable, and place an independent bounded review immediately after a cohesive high-risk invariant before downstream work depends on it. Run multiple review agents in parallel only when their review scopes are disjoint. For each distinct change-bearing author Agent, the coordinator SHOULD provide at least one independent review assignment covering that author's PROCESS outputs and affected SPECs. One reviewer MAY cover multiple authors, and final verification remains per SPEC rather than enforcing a 1:1 relation. The reviewing agent's --agent identity MUST name a real sub-agent actually spawned to perform the review; the coordinator MUST NOT self-author review findings or fabricate an agent name to bypass this. Repairs extend or replace the invariant-owning PROCESS; create a separate repair PROCESS only for a genuinely independent invariant, then converge all fixes.
9. Only after independent review/fix convergence, each code author adds the backend-appropriate final rationale under its own logical identity by following issue-spec-workflow.
...
11. Follow issue-spec-workflow for backend-appropriate merge and closure, preserving the managed closure block's final-write rule when that route uses it.
`,
		},
		{
...
1. Before inspecting code, the review agent validates its required ` + "`design_context`" + `, reads the complete Design issue body with ` + "`issue-spec read issue --repo {{repo}} --issue <design_context.source_url>`" + ` without comments, timeline, history, or gates, and stops if that authoritative body conflicts with any projected field. It accepts only ` + "`read_mode: complete-issue-body`" + ` and ` + "`conflict_policy: design-authoritative-stop`" + `; session identifiers are optional audit metadata, not correctness inputs. The review agent then runs issue-spec review sync --repo {{repo}} --implement <issue> --id REVIEW-<n> --agent <review-agent> --agent-session <id> --json under its own identity. On GitHub add --pr <number>; on a self-hosted profile omit --pr and add --revision <exact-head>. Sync authoritatively captures current rationale, findings, checks, per-PROCESS execution class and evidence diagnostics, then upserts one stable done REVIEW completion even with zero findings. The evidence gate reads the REVIEW writer's --agent as the reviewing identity for independence, so the reviewing agent (never the coordinator) MUST author it. review sync owns the established "## Review Sync Summary" REVIEW body and completion evidence: never hand-edit either, fabricate a finding, or substitute a generic approval framework. For separate manual review evidence, generate a REVIEW body with issue-spec comment generate --type REVIEW --input-file review.json and upsert it under the same review-agent identity.
...
4. Follow issue-spec-workflow for backend-appropriate finding routing. When that route uses issue-spec review finding, each review agent authors its own actionable line findings under its own --agent identity and assigned --agent-session. Use P0/P1 for blockers and P2 for non-blocking follow-up. The coordinator does not create findings on a review agent's behalf.
...
6. Follow issue-spec-workflow for backend-appropriate reply and resolution routing. The worker that owns the affected code fixes it and replies on the original finding thread using its own --agent and --agent-session. The review agent that opened the finding then re-checks the diff and owns the resolved reply or code-review conversation resolution; a worker reply alone does not resolve a finding.
...
6. Final verify must fail if blocking questions, missing links, missing class-specific PROCESS evidence, open P0/P1 findings, failed or pending PR checks, or durable spec omissions exist. Change-bearing PROCESS nodes require backend-appropriate rationale and REVIEW completion evidence. Status forecast and final verify use the same authoritative validator for that rationale and completion evidence, required traceability, and reviewer independence. The validator owns exact identity, revision, freshness, and legacy compatibility. Neither command creates, updates, or refreshes REVIEW. Review, verification, orchestration, and external nodes use their proportional evidence carriers.
`,
		},
		{
