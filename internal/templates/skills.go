...
11. Add final PR rationale only after review/fix convergence and only for change-bearing PROCESS nodes on GitHub. Once all P0/P1 findings are resolved, the coordinator dispatches each owning worker to add rationale on the key code blocks it owns, authored under that worker's own --agent and --agent-session. On self-hosted profiles, keep review and exact-revision evidence on the selected provider and use the attached code-change relationship for navigation; do not invent GitHub PR rationale. The coordinator MUST NOT create worker rationale or relabel its identity on the worker's behalf. Every class still requires TASK, implementation-change, and active SPEC traceability.
...
9. Only after independent review/fix convergence, each code author adds final PR rationale for its owned key code blocks under its own logical identity on GitHub. Self-hosted workflows keep review and exact-revision evidence on the selected provider instead of inventing GitHub PR rationale.
