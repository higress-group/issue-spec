...
9. Only after independent review/fix convergence, have each code author add final PR rationale on GitHub. For self-hosted profiles, the code author runs issue-spec code-change rationale against the Implement Issue under its own --agent and --agent-session; final verification accepts it only when its provider/repository/change, active reference version, subject revision, PROCESS, and SPEC exactly match trusted consumed native-ledger evidence. This is an Issue Backend comment and never a GitHub PR endpoint.
