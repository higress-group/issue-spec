...
4. Upsert ` + "`proposal-choice-brief`" + ` after that pass and before complete SPEC authoring. It is coverage-complete rather than delta-only: cover the problem, outcome, success signal, boundaries, non-goals, assumptions, risks, decisions, alternatives, and expected SPEC coverage. It distinguishes settled, needs-evidence, and needs-decision items, with grounded recommendations, alternatives, tradeoffs, and source links. With no open decision, keep the other review dimensions visible. The projection is ordinary and statusless.
...
6. Persist the authoritative self-contained Design, perform its first QUESTION discovery/create pass, then upsert ` + "`design-explainer`" + ` before complete TASK planning. Cover architecture, every invariant, end-to-end flow, failure and trust boundaries, interfaces, shared state, state transitions, alternatives, compatibility, rollout, rollback, risks, verification, and active SPEC traceability. Use purposeful interaction to make the complete review surface easier to navigate.
...
Coordinator: persist the Implement issue, perform its first QUESTION discovery/create pass, then upsert the ordinary statusless ` + "`implement-execution-brief`" + ` before completing PROCESS planning. The brief is a coverage-complete current execution review, not an incremental planning note. It explains invariant-based PROCESS candidates or the current DAG, state counts, critical path, safe parallelism, Agent allocation and role reasons, per-node SPEC and scenario coverage, code-volume estimates with confidence, correctness complexity, shared touchpoints, blockers, tests, generators, and independent review/verify obligations. Correctness complexity is distinct from size, and estimates never define workflow semantics. Issue bodies and typed artifacts remain authoritative, and projection source stays outside default Agent context.
