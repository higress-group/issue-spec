...
- A bounded implementation MAY use exactly one code-writing child or subagent without PROCESS when the coordinator does not write concurrently and none of the managed-coordination needs above applies. Read-only investigation and review children never require PROCESS. Do not create PROCESS solely because a child is used, several files change, independent review is desired, or human handoff is needed.
