...
2. Coding MAY be inline or delegated, and the two paths differ in workspace mechanics. Delegated: declare workspace_management: managed and run workspace prepare -> runtime-native child -> complete -> integrate. Inline: declare workspace_management: independent, implement/test/commit in the coordinator's own integration checkout, and do NOT run prepare/child/complete/integrate for that node. Both paths first produce commit/test evidence and, for a serial predecessor, a bounded ### Handoff; once the code is integrated or otherwise reviewable, both paths MUST schedule an independent review PROCESS and converge any fixes. Only after review/fix convergence does the code author add final PR rationale. Delegation is a recommended technique for keeping the coordinator context bounded and avoiding mid-task compaction on large nodes, not a requirement; inline coding is allowed for any node because independent review is mandatory regardless.
