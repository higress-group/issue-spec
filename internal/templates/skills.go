...
- Non-trivial changes MUST include a dedicated review PROCESS node in the DAG, and the review MUST be performed by a different agent than the code author; the code author (coordinator or worker) MUST NOT review its own code. Independence is judged by the reviewing agent's --agent identity, which MUST name a real sub-agent actually spawned to perform the review and MUST NOT be a fabricated or reused name used only to pass this check. Review agents are scheduled like worker agents and can run in parallel when their review scopes are independent.
...
2. Coding MAY be inline or delegated: the coordinator MAY implement a non-trivial coding node itself or delegate it to a worker sub-agent, serial or parallel. Delegation is a recommended technique for keeping the coordinator context bounded and avoiding mid-task compaction on large nodes, not a requirement. Inline coding is allowed for any node because independent review is mandatory.
...
5. Default to serial PROCESS chains under one parent TASK. When a serial node is delegated, run each node in its own worker connected by a bounded ### Handoff. Each completed serial PROCESS records ### Handoff evidence (the contract/state its successor consumes) before the next node starts, seeding the successor worker with the parent TASK context plus the predecessor ### Handoff rather than the coordinator's accumulated context; record a reason when a handoff is unnecessary.
...
7. Dispatch review PROCESS nodes for non-trivial PRs after PR rationale exists, each owned by an agent that did not author the code under review; run review nodes in parallel only when their review scopes are independent. Route P0/P1 findings to the owner PROCESS or a dedicated repair PROCESS that follows the same serial/parallel gating.
...
3. Plan the PROCESS DAG before dispatch. The coordinator MAY implement coding nodes inline or delegate them to a worker sub-agent. Delegation is a recommended technique for keeping the coordinator context bounded and avoiding mid-task compaction on large or context-heavy nodes, not a requirement; when delegated, the coordinator plans and integrates while workers hold the bulk coding context. Inline coding is allowed for any node because independent review (step 5) is MANDATORY: whoever wrote the code MUST NOT be the one who reviews it. Read each active TASK's ### Execution Planning metadata and derive PROCESS nodes from it. Render PROCESS bodies with issue-spec comment generate --type PROCESS --input-file process.json (fields include parent_task, execution_class, workspace_management, owner, dependencies, write_ownership, handoff) instead of hand-writing Markdown; comment upsert --type PROCESS rejects a PROCESS without a ### Parent TASK. Use managed workspace management by default and for runner or multi-agent execution. Independent means issue-spec did not allocate the implementation worktree; it may be operated by a coordinator, human, or external executor. It changes workspace allocation only, not the workflow's existing review or verification policy; plan a separate REVIEW PROCESS for non-trivial independent changes. A missing legacy declaration remains conservatively managed.
...
3. Coding MAY be inline or delegated: the coordinator MAY implement a coding node itself or delegate it to a worker sub-agent to keep the coordinator context bounded and avoid mid-task compaction on large nodes. Delegation is a recommended technique for context isolation, not a requirement.
...
7. Non-trivial PRs MUST be reviewed by a review agent that is not the author of the code under review; spawn or assign it, and run multiple review agents in parallel only when their review scopes are disjoint. The reviewing agent's --agent identity MUST name a real sub-agent actually spawned to perform the review; the coordinator MUST NOT self-author review findings for code it or a worker wrote, nor fabricate an agent name to bypass this. Route findings to the owner PROCESS or a dedicated repair PROCESS under the same serial/parallel gating.
