...
4. Implementation and review packets require design_context. The role reads the complete canonical Design body at design_context.source_url with issue-spec read issue --repo {{repo}} --issue <url>, without comments, timeline, history, or gates, and stops on any conflict. Runtime session values, including CODEX_THREAD_ID and --agent-session, are optional audit metadata only.
...
2. Create phase issues with concrete body files, beginning with issue-spec issue create proposal --repo {{repo}} --body-file <file>. Use the standardized Proposal:, Design:, and Implement: title family. Use --title only for an explicitly requested custom title; do not perform style-only title rewrites.
...
3. Work only in the assigned worktree and owned paths. Preserve the named invariant, decisions, must_preserve, must_not, and minimum_verification exactly. CODEX_THREAD_ID and --agent-session are optional audit metadata, never authority or correctness inputs.
...
2. Require design_context.read_mode=complete-issue-body and conflict_policy=design-authoritative-stop. Before inspecting code, read the complete Design with issue-spec read issue --repo {{repo}} --issue <design_context.source_url> without comments, timeline, history, or gates. Stop on conflict; session ids are optional audit metadata.
...
3. Generate/submit the bounded VERIFY receipt under the real verifier identity. Record command/check identity, revision, result, and failures. CODEX_THREAD_ID and --agent-session are optional audit metadata, not verification authority.
