...
- Treat the exact PROCESS id as the workspace selector. For a delegated checkout, use issue-spec workflow workspace prepare, inspect, complete, integrate, reconcile, and cleanup with the same --repo, --issue, --process, integration root, workspace root, and owner token. change-bearing uses a writable owned branch; review and verification use detached snapshots and fail closed when dirty. A standalone snapshot is not OS-immutable; Linux runner bubblewrap adds a read-only bind. orchestration uses bookkeeping with no checkout. The standalone CLI currently records external as mode none, while runner external execution additionally requires its configured provider adapter to pass readiness before acpx starts. Never infer a PROCESS from prompt text.
- In runner mode, a restart must reconcile the durable PROCESS assignment before execution and fail closed on a missing, dirty, mismatched, or needs-reconcile workspace. Runner terminal, cancellation, and reconciliation paths record cleanup intent durably and apply integration/retention eligibility before cleanup, retrying pending cleanup. By contrast, standalone workflow workspace cleanup is an explicit owner-token-authorized destructive operation: it does not apply runner integration/retention eligibility and can remove unintegrated change-bearing work, so invoke it only after the intended integration or retention decision. Target runner work with /resume <public-session-id> --process PROCESS-NNN <instruction>; --process must immediately follow the public session id and cannot be used with /new.
...
- Before allocating a delegated worker, run issue-spec doctor agent --repo {{repo}} --operation <operation> --json for every required provider-neutral operation (issue.read, artifact.write, pr.read, pr.review.write, checks.read, git.clone, git.push, or external.change.comment). Strict runner work requires an operator-owned short-lived issuer; legacy_long_lived mirrored gh credentials are compatibility-only and never satisfy strict operation policy.
...
7. Allocate only the exact PROCESS workspace selected by the runner or coordinator. Use issue-spec workflow workspace prepare, inspect, complete, integrate, reconcile, and cleanup with stable --repo, --issue, --process, integration root, workspace root, and owner token values. change-bearing receives a writable owned branch; review and verification receive detached snapshots and fail closed when dirty. Standalone snapshots are not OS-immutable; Linux runner bubblewrap binds them read-only. orchestration records no-checkout bookkeeping. The standalone CLI records external as mode none, while runner external execution additionally gates on its configured provider adapter. Runner restart reconciles the durable assignment and fails closed on drift; runner terminal cleanup persists intent, applies integration/retention eligibility, and retries pending cleanup. Standalone workflow workspace cleanup does not apply that eligibility: it is owner-token-authorized, destructive, and can remove unintegrated change-bearing work, so run it only after the intended integration or retention decision. To target a PROCESS in runner mode use /resume <public-session-id> --process PROCESS-NNN <instruction>; never infer the selector from prompt text.
8. Implement the code changes for one PROCESS scope at a time, or integrate completed worker outputs by dependency order. The worker that owns a code scope owns its own commits; the coordinator does not author code artifacts on a worker's behalf unless it is the assigned worker.
