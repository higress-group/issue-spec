...
- On self-hosted profiles, the real review agent runs ` + "`issue-spec --profile <self-hosted-profile> review sync --repo {{repo}} --implement <issue> --revision <revision> --id REVIEW-001 --agent <review-agent> --json`" + `. A successful sync persists and reloads provider facts, then upserts one stable done REVIEW with an exact-current completion stamp even when there are zero findings. Never fabricate a finding or hand-author the completion stamp.
...
- Treat Agent as the logical role or workflow-assigned label. Artifact writer session metadata is deprecated, ignored, and never required.
- When dispatching subagents, pass only the bounded role, scope, revision, and evidence contract required for their work; do not collect runtime-specific session ids.
- When runner context supplies runner.public_session_id, it is the public /resume handle. Coordinator-authored proposal, design, implement, handoff, and update issue bodies or comments should include runner.public_session_id and /resume <public-session-id> <answer or next instruction> when available. Do not present coordinator record ids or provider session ids as /resume handles.
...
4. Select ready PROCESS nodes whose dependencies are done. Dispatch every agent-executed change-bearing node and review node to its assigned real worker or review agent with only its bounded role, scope, revision, and evidence contract. One real worker MAY execute multiple compatible serial change-bearing or code-repair nodes; a fresh worker is not required for every PROCESS, but each node retains distinct status, dependencies, workspace lifecycle, evidence, and handoff. Parallelism remains a separately gated optimization: run independent nodes concurrently only when write ownership is provably disjoint.
...
9. Only after independent review/fix convergence, have each code author add final PR rationale on GitHub. For self-hosted profiles, the code author runs issue-spec code-change rationale against the Implement Issue under its own --agent; final verification accepts it only when its provider/repository/change, active reference version, subject revision, PROCESS, and SPEC exactly match a fresh REVIEW completion carrier or a valid existing finding-backed consumed native-ledger binding. This is an Issue Backend comment and never a GitHub PR endpoint.
...
3. Plan the PROCESS DAG before dispatch. Every agent-executed change-bearing node MUST declare workspace_management: managed and run workspace prepare -> real runtime-native child -> complete -> integrate; the coordinator plans and integrates while the worker owns implementation, focused tests, the result commit, and bounded handoff. The worker's logical Agent MUST differ from the PROCESS coordinator Agent. A different Agent name without a real dispatched child is insufficient and MUST NOT be fabricated or relabeled only to pass process.executor.coordinator_conflict. The coordinator MUST NOT implement/test/commit such a node inline or use workspace_management: independent as an escape hatch. independent remains the general self-managed mode for external or human executors that genuinely own their workspace. Each change-bearing node first produces commit/test evidence and, for a serial predecessor, a bounded ### Handoff; then independent review and any fixes converge under step 5; only afterward does the code author add final rationale under step 11. Read each active TASK's ### Execution Planning metadata and derive PROCESS nodes from it. Render PROCESS bodies with issue-spec comment generate --type PROCESS --input-file process.json (fields include parent_task, execution_class, workspace_management, owner, dependencies, write_ownership, handoff) instead of hand-writing Markdown; comment upsert --type PROCESS rejects a PROCESS without a ### Parent TASK. A missing legacy declaration remains conservatively managed.
   Keep Agent as the logical role. Do not collect or pass runtime-specific session ids.
...
5. Parallelism is a separate, gated optimization, not the trigger for delegation: run independent worker nodes concurrently only when their write ownership is provably disjoint.
...
1. The review agent runs issue-spec review sync --repo {{repo}} --implement <issue> --id REVIEW-<n> --agent <review-agent> --json under its own identity. On GitHub add --pr <number>; on a self-hosted profile omit --pr and add --revision <exact-head>. Sync authoritatively captures current rationale, findings, checks, per-PROCESS execution class and evidence diagnostics, then upserts one stable done REVIEW completion even with zero findings. The evidence gate reads the REVIEW writer's --agent as the reviewing identity for independence, so the reviewing agent (never the coordinator) MUST author it. review sync owns the established "## Review Sync Summary" REVIEW body and completion evidence: never hand-edit either, fabricate a finding, or substitute a generic approval framework. For separate manual review evidence, generate a REVIEW body with issue-spec comment generate --type REVIEW --input-file review.json and upsert it under the same review-agent identity.
