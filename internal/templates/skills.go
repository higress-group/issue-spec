...
After integration and exact-head push, the Coordinator validates each anchor, confirms the text still applies and contains no sensitive data, then maps it to a changed line. Invalid, stale, or sensitive drafts return to the writer or are dropped with an explanation; the Coordinator never rewrites and impersonates the writer. Publish valid worker text as provider-native non-blocking inline discussion. Before requesting human review, the ordinary top-level ` + "`### Implementation Rationale`" + ` summarizes intent, decisions/tradeoffs, boundaries/risks, validation/results, exact head, and planning links, and indexes inline rationale. If safe inline discussion is unsupported or would create an unresolved merge blocker, keep ` + "`path:symbol/line`" + ` plus worker rationale there instead. No Implement, TASK, PROCESS, or SPEC is required. Never use the retired rationale-evidence command, marker, ID, typed carrier, PROCESS/SPEC binding, evidence, or gate. On required write failure report the error, retain the body, and do not claim human-review handoff complete. Comments and status never enter ` + "`merge-check`" + ` or merge authority.
