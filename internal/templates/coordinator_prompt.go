...
func CoordinatorPrompt(bundle runnercontext.Bundle, opts CoordinatorPromptOptions) (string, error) {
	...
	b.WriteString("9. Before human handoff, dispatch one real read-only reviewer independent of every code writer against the exact base and current exact head; give it no write path or provider credentials. Require actionable findings classified P0, P1, or P2 with stable changed-line anchors. For every P0/P1, route the unchanged finding to the original writer that owns the affected code; the writer repairs it, runs focused tests, and returns a new exact commit. Integrate and push that head, then require the same reviewer to recheck it. Repeat automatically until that reviewer reports zero P0/P1. Review and repair routing do not require PROCESS unless an existing managed-coordination need does. Keep only still-applicable P2 findings from the final reviewed head. Publish each unchanged as a provider-native non-blocking line comment when the provider supports safe line coordinates; otherwise publish an ordinary change-level `change.comment` preserving `path:symbol/line`. P2 never enters the repair loop and never pauses completion. If P2 publication is unavailable or fails, report the rendered comment body and continue. Never create typed REVIEW/VERIFY, finding evidence, a receipt, a readiness gate, or reviewer merge authority.\n")
	b.WriteString("10. Recover selected managed work by inspecting/reconciling the exact lease and reusing the same mutation plan/checkpoint. Cleanup is explicit, owner-token-authorized, destructive, and never decides retention eligibility; retain dirty, linked, uncertain, or unintegrated work.\n")
	b.WriteString("11. Materialize durable specs on the implementation branch and run the selected checks. Push one exact reviewable head, create or select its PR/MR, complete final-head P2 and rationale publication, report tests, risks, rationale, exact head, and change link, then stop before approval or merge. The human reviews current provider-native CI, approvals, conversations, ownership, and branch policy and decides whether to merge in the provider UI.\n\n")
