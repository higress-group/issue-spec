...
func CoordinatorPrompt(bundle runnercontext.Bundle, opts CoordinatorPromptOptions) (string, error) {
	...
	b.WriteString("- The top-level runner recovers only the ACPX/session job after resume or restart. For delegated (managed) PROCESS nodes, this coordinator owns the workspace lifecycle: from the unchanged session clone, inspect or reconcile the exact managed PROCESS lease before complete and integrate. Inline (independent) nodes have no PROCESS lease and skip prepare/child/complete/integrate. Invoke owner-token cleanup only after an explicit integration or retention decision. Runner session-clone retention consults `git worktree list` and fails closed by retaining the clone when runner metadata is dirty or uncertain, a linked worktree exists, or git worktree inspection fails; the runner does not own, persist, or retry child PROCESS cleanup.\n")
	...
	fmt.Fprintf(&b, "- After a delegated managed child returns, validate its result commit, tests, and handoff; then run `%s workflow workspace complete` and `integrate` from the unchanged coordinator checkout before status/link/handoff updates. This completion/integration lifecycle applies only to delegated managed outputs; inline independent nodes skip it. `workflow workspace cleanup` is an explicit owner-token destructive operation; it does not decide or enforce integration/retention eligibility for you.\n", issueSpec)
	...
	b.WriteString("- On implement and apply turns, plan the PROCESS DAG first, then execute each coding node either delegated or inline. Delegated: declare the node `workspace_management: managed` and run `workflow workspace prepare` -> child/worker -> `complete` -> `integrate`. Inline coordinator-authored: declare the node `workspace_management: independent` and implement, test, and commit in the integration checkout yourself, skipping prepare/child/complete/integrate. `workspace_management: independent` remains the general self-managed mode and MAY instead use an external or human executor's own workspace; it is not restricted to coordinator-inline execution. Every path first produces commit/test evidence and any serial-predecessor handoff, then routes reviewable code through mandatory independent review and fix convergence (see Review), and only afterward has the code author add final PR rationale under its own identity. Inline coding is allowed for a node of any size because independent review catches what the author cannot.\n")
	b.WriteString("- Delegation is a recommended technique, not a requirement: delegate to keep the coordinator context bounded and avoid mid-task compaction on large or context-heavy nodes, serial or parallel.\n")
	b.WriteString("- When a serial node is delegated, run it in its own worker connected by a bounded `### Handoff`, and seed a successor worker with the parent TASK context plus the predecessor `### Handoff`, never the coordinator's accumulated context. When a serial node is inline, execute it in the integration checkout but retain distinct per-PROCESS state and the same bounded `### Handoff` boundary.\n")
	...
	fmt.Fprintf(&b, "- For delegated nodes, stay lean: the coordinator retains only orchestration, gate-evaluation, integration, and handoff state, and consumes bounded worker outputs and `%s read` results, not full issue/PR bodies or full diffs. For an inline node, it additionally holds that node's bounded implementation, test, commit, and handoff context.\n", issueSpec)
	b.WriteString("- Review is a first-class PROCESS node, not an inline coordinator step: every active SPEC that has a valid change-bearing carrier MUST be covered by at least one independent review PROCESS, completed before final verify. A single review PROCESS MAY cover a SPEC that several implementation PROCESS nodes contribute to; do not force a 1:1 review-per-implementation-node pairing. Small changes MAY be implemented inline by the coordinator, but they still MUST be reviewed by a different agent -- the code author (coordinator or worker) MUST NOT review its own code. Independence is judged by the reviewing agent's `--agent` identity, which MUST name a real sub-agent actually spawned to perform the review and MUST NOT be a fabricated or reused name used only to pass this check. Final verify fails closed if no independent review PROCESS covers a change-bearing SPEC. Route findings to the owner PROCESS or a dedicated repair PROCESS under the same serial/parallel gating.\n")
	...
	b.WriteString("- Identify ready PROCESS/review nodes, execute inline nodes in the coordinator, dispatch only delegated managed coding nodes and review nodes, integrate delegated outputs by dependency order, and record evidence before marking work done.\n\n")
