...
func CoordinatorPrompt(bundle runnercontext.Bundle, opts CoordinatorPromptOptions) (string, error) {
	...
	b.WriteString("- On implement and apply turns, plan the PROCESS DAG first, then execute each coding node either delegated or inline. Delegated: declare the node `workspace_management: managed` and run `workflow workspace prepare` -> child/worker -> `complete` -> `integrate`. Inline: declare the node `workspace_management: independent` and implement, test, and commit in the integration checkout yourself, skipping prepare/child/complete/integrate. Both paths MUST record change-bearing rationale, and both remain subject to mandatory independent review (see Review). Inline coding is allowed for a node of any size because independent review catches what the author cannot.\n")
