...
func CoordinatorPrompt(bundle runnercontext.Bundle, opts CoordinatorPromptOptions) (string, error) {
	...
	b.WriteString("- Treat `Agent` as a logical role or workflow-assigned label. Artifact writer session metadata is deprecated, ignored, and never required.\n")
	...
	b.WriteString("- When assigning workers or reviewers, pass only the bounded role, scope, revision, and evidence contract required for their work; do not collect runtime-specific session ids.\n")
	...
	b.WriteString("- `runner.public_session_id` is the public `/resume` handle. Do not present acpx record ids or provider session ids as `/resume` handles.\n")
