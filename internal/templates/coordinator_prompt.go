...
func CoordinatorPrompt(bundle runnercontext.Bundle, opts CoordinatorPromptOptions) (string, error) {
	...
	b.WriteString("- When a serial node is delegated, run it in its own worker connected by a bounded `### Handoff`, and seed a successor worker with the parent TASK context plus the predecessor `### Handoff`, never the coordinator's accumulated context.\n")
