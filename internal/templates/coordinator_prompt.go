...
func CoordinatorPrompt(bundle runnercontext.Bundle, opts CoordinatorPromptOptions) (string, error) {
	...
	fmt.Fprintf(&b, "- Prefer `%s comment get` and filtered planning lists. Use `%s merge-check ... --json` only after the exact provider subject and configured authorities exist; it is read-only current output, never saved proof.\n", issueSpec, issueSpec)
	...
	b.WriteString("7. For selected PROCESS work, validate result revision, DCO, ownership, generators, tests, and receipt binding through CLI lifecycle commands. Integrate by dependency order and record only bounded handoff/evidence. Seed serial successors with parent TASK plus predecessor handoff, never accumulated Coordinator context.\n")
	b.WriteString("8. Before requesting human review of a reviewable exact change, publish or refresh one ordinary top-level provider discussion headed `### Implementation Rationale` for both direct single-writer and managed PROCESS paths. Use the provider-native discussion surface; never create a typed carrier, PROCESS/SPEC binding, rationale ID, evidence, or gate. Report publication failure with the rendered body and do not claim review handoff complete. This mutable review aid and its publication status never enter merge-check or merge authority. Provider-native review and current configured checks alone own authority; never fabricate reviewer identity or copy decisions into REVIEW/VERIFY comments. Route authority blockers and rerun read-only merge-check.\n")
	...
	b.WriteString("10. Materialize durable specs on the implementation branch as an ordinary configured check. Merge only through code-change merge with caller-observed expected head and a freshly collected provider authority token; ordinary GitHub REST read-then-write remains fail-closed. Reconcile the exact selected Issue set idempotently after freshly observed merge.\n\n")
