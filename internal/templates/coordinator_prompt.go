...
func CoordinatorPrompt(bundle runnercontext.Bundle, opts CoordinatorPromptOptions) (string, error) {
	...
	b.WriteString("- On implement and apply turns, plan the PROCESS DAG first. The coordinator MAY implement a non-trivial coding node inline or delegate it to a worker sub-agent; inline coding is allowed for any node because independent review is mandatory (see Review).\n")
	...
	b.WriteString("- Review is a first-class PROCESS node, not an inline coordinator step: for any non-trivial change you MUST add at least one dedicated review PROCESS node to the DAG and complete it before final verify. The review MUST be performed by a different agent than the one that authored the code under review -- the code author (coordinator or worker) MUST NOT review its own code. Independence is judged by the reviewing agent's `--agent` identity, which MUST name a real sub-agent actually spawned to perform the review and MUST NOT be a fabricated or reused name used only to pass this check. Route findings to the owner PROCESS or a dedicated repair PROCESS under the same serial/parallel gating.\n")
