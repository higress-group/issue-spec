...
func CoordinatorPrompt(bundle runnercontext.Bundle, opts CoordinatorPromptOptions) (string, error) {
	...
	fmt.Fprintf(&b, "- The runner preflights the selected issue backend inside the sandbox. For conversational replies, status updates, clarification, recommendations, and handoff, use the selected issue backend or `%s comment upsert` for typed artifacts; do not assume a GitHub CLI login can write the selected issue service.\n", issueSpec)
