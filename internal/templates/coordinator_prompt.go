...
func CoordinatorPrompt(bundle runnercontext.Bundle, opts CoordinatorPromptOptions) (string, error) {
	...
	b.WriteString("- Agent is a logical role. CODEX_THREAD_ID, --agent-session, and provider session values are optional audit metadata only; none is authority, a gate input, or the public resume handle. runner.public_session_id alone is the /resume handle.\n")
