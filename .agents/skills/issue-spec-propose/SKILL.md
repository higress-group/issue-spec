...
4. Upsert `proposal-choice-brief` after that pass and before complete SPEC authoring. It distinguishes settled, needs-evidence, and needs-decision items, with grounded recommendations, alternatives, tradeoffs, and source links. The projection is ordinary and statusless.
...
6. Persist the authoritative self-contained Design, perform its first QUESTION discovery/create pass, then upsert `design-explainer` before complete TASK planning. Use purposeful interaction to explain data, flow, alternatives, and correctness constraints.
