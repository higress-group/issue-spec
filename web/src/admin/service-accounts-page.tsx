...
import { RepositoryScopeSelector, RUNNER_TOKEN_SCOPES, TOKEN_SCOPES, TokenScopeSelector } from "../auth/token-authority-fields";
...
type ManagedTokenForm = { name: string; scopes: string[]; repositories: string[] | null };
...
export function ManagedTokensPage() {
  ...
  const { register, handleSubmit, reset, setValue, getValues, control, formState: { errors } } = useForm<ManagedTokenForm>({ defaultValues: { name: "", scopes: [...TOKEN_SCOPES], repositories: null } });
  ...
  const create = useMutation({ mutationFn: (form: ManagedTokenForm) => api.createManagedPAT(orgId, target!.id, {
    name: form.name,
    scopes: form.scopes,
    repository_ids: form.repositories ?? [],
  ...
  return <div className="page"><PageHeader eyebrow={t("managedTokens.eyebrow")} title={t("managedTokens.title")} description={t("managedTokens.description")} />
    ...
    <div className="two-column token-layout"><Panel title={t("managedTokens.createTitle")} description={t("managedTokens.createDescription")}><form className="form-grid" onSubmit={handleSubmit((form) => create.mutate(form))}>
      ...
      <Controller name="repositories" control={control} rules={{ validate: (value) => value === null || value.length > 0 || t("tokenAuthority.repositoryRequired") }} render={({ field, fieldState }) => <RepositoryScopeSelector name="managed-token-repositories" options={(repositories.data?.repositories ?? []).map((repository) => ({ id: repository.id, label: repository.name }))} value={field.value} onChange={field.onChange} error={fieldState.error?.message} />} />
      <div className="button-row"><button className="button secondary" type="button" onClick={() => { if (!getValues("name")) setValue("name", "runner"); setValue("scopes", [...RUNNER_TOKEN_SCOPES], { shouldValidate: true }); }}><ShieldCheck size={16} />{t("managedTokens.runnerPreset")}</button><button className="button primary" type="submit" disabled={!target || create.isPending}><KeyRound size={16} />{t("managedTokens.create")}</button></div>
