...
export function MembersPage() {
  ...
  const invite = useMutation({ mutationFn: (form: InviteForm) => { if (!resolved) throw new Error(t("members.resolveFirstError")); return api.inviteMembership(orgId, { user_id: resolved.id, role: form.role }); }, onSuccess: () => { setResolved(undefined); void refresh(); }, onError: inspector.report });
  ...
  return <div className="page"><PageHeader eyebrow={t("members.eyebrow")} title={t("members.title")} description={t("members.description")} />
    <Panel title={t("members.inviteTitle")} description={t("members.inviteDescription")}><form className="inline-form" onSubmit={handleSubmit((form) => invite.mutate(form))}><Field label={t("common.localLogin")} error={errors.login?.message}><div className="input-action"><TextInput {...register("login", { required: t("members.loginRequired"), onChange: () => setResolved(undefined) })} /><button className="icon-button" type="button" onClick={() => resolve.mutate(getValues("login"))} aria-label={t("common.resolveLogin")}><Search size={17} /></button></div></Field><Field label={t("common.role")}><SelectInput {...register("role")}><option value="reader">{t("common.permission.reader")}</option><option value="member">{t("common.permission.member")}</option><option value="maintainer">{t("common.permission.maintainer")}</option><option value="owner">{t("common.permission.owner")}</option></SelectInput></Field><div className="resolve-state">{resolved ? <StatusBadge tone="teal">{t("common.resolvedLogin", { login: resolved.login })}</StatusBadge> : <span>{t("members.resolveBeforeInvite")}</span>}</div><button className="button primary" type="submit" disabled={!resolved || invite.isPending}><UserPlus size={16} />{t("members.invite")}</button></form></Panel>
