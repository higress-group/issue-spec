...
export function CollaboratorsPage() {
  ...
  return <div className="page"><PageHeader eyebrow="Repository / access" title="Collaborators" description="Repository grants raise authority explicitly; token scopes can still cap them." />
    ...
    <Panel title="Explicit grants">{collaborators.error ? <ErrorNotice error={collaborators.error} /> : null}<div className="resource-list">{collaborators.data?.collaborators.map((collaborator) => { const user = names.get(collaborator.user_id); return <article className="resource-row" key={collaborator.id}><div><strong>{user?.display_name ?? "Unknown local user"}</strong><span className="mono">@{user?.login ?? collaborator.user_id}</span></div><StatusBadge tone="purple">{collaborator.role}</StatusBadge><button className="icon-button danger-text" type="button" onClick={() => remove.mutate({ id: collaborator.id, version: collaborator.representation_version })} aria-label={`Remove ${user?.login ?? "collaborator"}`}><Trash2 size={17} /></button></article>; })}</div></Panel>
  </div>;
}
