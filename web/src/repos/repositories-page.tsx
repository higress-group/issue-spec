...
type ContributionPolicy = "disabled" | "members" | "authenticated" | "public";
type RepoForm = { name: string; display_name: string; description: string; visibility: RepositoryVisibility; default_branch: string; contribution_policy: ContributionPolicy };

export function suggestedContributionPolicy(visibility: RepositoryVisibility, current: ContributionPolicy, explicitlySelected: boolean): ContributionPolicy {
  if (explicitlySelected) return current;
  return visibility === "public" ? "public" : "members";
}
...
export function RepositoriesPage() {
  ...
  const create = useMutation({ mutationFn: (form: RepoForm) => api.createRepository(orgId, form), onSuccess: () => { setContributionPolicyExplicit(false); reset(); void client.invalidateQueries({ queryKey: queryKeys.repoContext(orgId) }); }, onError: inspector.report });
  const visibilityInput = register("visibility", { onChange: (event) => {
    setValue("contribution_policy", suggestedContributionPolicy(event.target.value as RepositoryVisibility, getValues("contribution_policy"), contributionPolicyExplicit));
  } });
  ...
  return <div className="page"><PageHeader eyebrow={t("repositories.eyebrow", { name: organization?.name ?? t("common.organization") })} title={t("repositories.title")} description={t("repositories.description")} actions={canCreate ? <a className="button secondary" href="#create-repository"><Plus size={16} />{t("repositories.new")}</a> : undefined} />
    ...
    {canCreate ? <Panel title={t("repositories.createTitle")} description={`${t("repositories.createDescription")} ${publicContributionHint}`} className="anchor-panel"><form id="create-repository" className="form-grid wide" onSubmit={handleSubmit((form) => create.mutate(form))}><Field label={t("common.name")} error={errors.name?.message}><TextInput {...register("name", { required: t("repositories.nameRequired") })} /></Field><Field label={t("common.displayName")} error={errors.display_name?.message}><TextInput {...register("display_name", { required: t("repositories.displayNameRequired") })} /></Field><Field label={t("common.description")}><TextInput {...register("description")} /></Field><Field label={t("common.defaultBranch")}><TextInput {...register("default_branch")} /></Field><Field label={t("common.visibility")}><SelectInput {...visibilityInput}><option value="private">{t("common.visibilityValue.private")}</option><option value="internal">{t("common.visibilityValue.internal")}</option><option value="public">{t("common.visibilityValue.public")}</option></SelectInput></Field><Field label={t("repositories.contributionPolicy")} hint={publicContributionHint}><SelectInput aria-label={t("repositories.contributionPolicy")} {...contributionPolicyInput}><option value="disabled">{t("common.contribution.disabled")}</option><option value="members">{t("common.contribution.members")}</option><option value="authenticated">{t("common.contribution.authenticated")}</option><option value="public">{t("common.contribution.public")}</option></SelectInput></Field><button className="button primary" type="submit" disabled={create.isPending}><Settings2 size={16} />{t("repositories.create")}</button></form></Panel> : null}
  </div>;
}
