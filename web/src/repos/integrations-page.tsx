    ...
    <Panel title="Webhook routes" description="Pause is resumable. Revoke is terminal and preserves configuration and delivery history for audit only.">
      ...
      <div className="webhook-list">{items.map((item) => {
        ...
        return <article className={`webhook-card ${lifecycle}`} key={item.id}>
          <header><span className={`pulse ${lifecycle}`} aria-hidden="true" /><div><strong className="break-word">{item.url}</strong><span>Repository route · v{item.representation_version} · updated {formatDate(item.updated_at)}</span></div><StatusBadge tone={revoked ? "coral" : item.active ? "teal" : "neutral"}>{lifecycle}</StatusBadge></header>
          <div className="event-strip">{item.event_types.map((event) => <span key={event}>{event}</span>)}</div>
...
function validateWebhookURL(value: string) {
  if (value.trim() !== value || value.includes("?") || value.includes("#") || value.includes("\\")) return "Use a canonical URL without credentials, query, or fragment";
  try {
