...
function EditableComment({ comment, owner, repo, currentLogin, canContribute, canTriage }: { comment: IssueComment; owner: string; repo: string; currentLogin: string; canContribute: boolean; canTriage: boolean }) {
  ...
  return <article className="timeline-card" id={`issuecomment-${comment.id}`}><header><Avatar login={comment.user.login} src={comment.user.avatar_url} /><span><strong>@{comment.user.login}</strong><small>commented {formatRelative(comment.created_at)}</small></span>{canEdit && !editing ? <button className="quiet-action" type="button" onClick={() => setEditing(true)}><Pencil aria-hidden="true" />Edit</button> : null}</header><div className="timeline-body">{editing ? <CommentEditor initial={comment.body} submitLabel="Save comment" pending={mutation.isPending} error={mutation.error} onCancel={() => setEditing(false)} onSubmit={(body) => mutation.mutate(body)} /> : <MarkdownView source={comment.body} />}</div>{!editing ? <ReactionPicker owner={owner} repo={repo} commentId={comment.id} summary={comment.reactions} currentLogin={currentLogin} readOnly={!canContribute} /> : null}</article>;
}
...
export function IssueDetail({ active }: { active: ActiveRepository }) {
  ...
  return <div className="issue-page"><header className="detail-header"><div className="detail-title"><Link className="issue-back" to={repositoryIssuePath(active)}>← {owner} / {repo}</Link><h1>{item.title} <span>#{item.number}</span></h1><div className="detail-state-row"><span className={`state-badge ${item.state}`}><CircleDot aria-hidden="true" />{item.state}</span><span>@{item.user.login} opened this on {formatRelative(item.created_at)}</span><span>· {item.comments} {item.comments === 1 ? "comment" : "comments"}</span></div></div>{canTriage ? <div className="header-actions"><button className="issue-button" type="button" onClick={() => setEditing((value) => !value)}><Pencil aria-hidden="true" />{editing ? "Cancel edit" : "Edit"}</button><button className="issue-button" type="button" disabled={updateState.isPending} onClick={() => updateState.mutate(item.state === "open" ? "closed" : "open")}><RotateCcw aria-hidden="true" />{item.state === "open" ? "Close" : "Reopen"}</button></div> : null}</header>
    <MutationProblem error={updateState.error} />
    <div className="detail-grid"><div className="timeline"><article className="timeline-card issue-origin"><header><Avatar login={item.user.login} src={item.user.avatar_url} tone="coral" /><span><strong>@{item.user.login}</strong><small>wrote the opening note</small></span></header><div className="timeline-body">{editing ? <IssueEditor initial={{ title: item.title, body: item.body, labels: item.labels.map((label) => label.name) }} labels={labels.data ?? []} submitLabel="Save issue" pending={updateIssue.isPending} error={updateIssue.error} onCancel={() => setEditing(false)} onSubmit={(draft) => updateIssue.mutate(draft)} /> : item.body ? <MarkdownView source={item.body} /> : <p className="issue-empty-copy">No description was provided.</p>}</div></article>
      {comments.isLoading ? <IssueLoading label="Loading conversation" /> : comments.data?.map((comment) => <EditableComment key={comment.id} comment={comment} owner={owner} repo={repo} currentLogin={current.data?.user.login ?? ""} canContribute={canContribute} canTriage={canTriage} />)}
      {canContribute ? <section className="new-comment" aria-labelledby="new-comment-heading"><h2 id="new-comment-heading">Continue the conversation</h2><CommentEditor key={comments.data?.length ?? 0} pending={createComment.isPending} error={createComment.error} onSubmit={(body) => createComment.mutate(body)} /></section> : <section className="new-comment read-only-note" aria-label="Read-only access"><h2>Read-only conversation</h2><p>{active.authenticated ? "Your current repository permission does not allow comments or reactions." : "Sign in with repository access to comment or react."}</p>{active.authenticated ? null : <Link className="issue-button" to="/login" state={{ returnTo: repositoryIssuePath(active, item.number) }}>Sign in</Link>}</section>}</div>
      <aside className="issue-sidebar" aria-label="Issue metadata"><section><span className="issue-kicker purple">Labels</span><LabelChips labels={item.labels} />{canTriage ? <details><summary>Manage labels</summary><LabelSelector labels={labels.data ?? []} selected={item.labels.map((label) => label.name)} disabled={replaceLabels.isPending} onChange={(names) => replaceLabels.mutate(names)} /></details> : null}<MutationProblem error={replaceLabels.error} /></section><section className="issue-relationships" aria-labelledby="issue-relationships-heading"><span className="issue-kicker coral" id="issue-relationships-heading">Code changes</span>{relationships.isLoading ? <p className="relationship-status" role="status">Loading linked delivery…</p> : relationships.error ? <p className="relationship-status relationship-error" role="alert">Linked code changes are temporarily unavailable.</p> : <CodeChangeList relationships={relationships.data?.relationships ?? []} />}</section><section><span className="issue-kicker">Authority</span><p><LockKeyhole aria-hidden="true" />{active.repository.effective_permission}</p><small>{active.authenticated ? "Changes are checked again by the server." : "Public read access · mutations require sign in."}</small></section></aside></div>
  </div>;
}
