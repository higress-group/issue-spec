...
- `--workspace-retention <duration>` controls when real poll cycles remove expired, non-active managed session clones. The default is 7 days. Queued, dispatched, running, locked, and interrupted session jobs remain protected. Before deleting a clone, retention calls `git worktree list`; a linked worktree or inspection failure fails closed and retains the clone. This does not clean child PROCESS workspaces.
