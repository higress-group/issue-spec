...
When the selected plan contains Design or TASK, or the user requests an independent worker, the coordinator makes no code changes on delegated or managed paths. Without managed PROCESS, exactly one real non-Coordinator native child owns the bounded implementation in the session checkout. With managed PROCESS, each change-bearing work package has one real non-Coordinator child owner and distinct packages may run concurrently. Delegation alone does not create managed workspace state.

When managed coordination is selected, the coordinator chooses the exact ready PROCESS from the typed DAG and owns its lifecycle through `issue-spec workflow workspace prepare`, `inspect`, `complete`, `integrate`, `reconcile`, and `cleanup`, with a stable repository, issue, PROCESS, roots, and owner token. Runner mode provides trusted session-local defaults through `ISSUE_SPEC_PROCESS_INTEGRATION_ROOT` and `ISSUE_SPEC_PROCESS_WORKSPACE_ROOT`; a standalone coordinator passes explicit `--integration-root` and `--workspace-root`. `change-bearing` receives a writable owned branch; historical `review` and `verification` classes remain audit-only; `orchestration` receives no checkout; `external` uses mode `none`. PROCESS lifecycle is optional implementation safety and never delivery acceptance.
...
- `--storage-min-free-bytes <n>` sets a free-space admission guard on the workspace filesystem. Before a dispatched job acquires any session or workspace lock, the runner reads live free bytes; below the threshold it runs one safe storage reconciliation and re-checks, and when space is still low the job stays queued instead of failing. A statfs failure with a configured threshold also delays dispatch (fail closed). The default `0` disables the guard.
- `--storage-orphan-grace <duration>` controls how long an unmatched session runtime directory (no retained session and no ownership proof) is observed before an applied storage reconciliation may delete it. The default is 7 days (`168h`). Orphan PROCESS workspace pools are never auto-deleted.
