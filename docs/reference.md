...
Before changing or reviewing code, the assigned role reads the complete Design issue body with `issue-spec read issue --repo owner/repo --issue <design_context.source_url>` without comments, timeline, history, or gate expansion. If that body conflicts with the structured projection, the role stops and reports the conflict. `CODEX_THREAD_ID`, `--agent-session`, and other runtime session values remain optional audit metadata and are not Design authority or correctness inputs.

An imported result file is not an identity or provenance trust root. Its writer, subject, logical agent name, `CODEX_THREAD_ID`, `--agent-session`, credentials, and assurance labels are informational and cannot create accepted implementation receipt authority or satisfy a non-Coordinator/independence gate. Unverified imports and reserved assurance values may be structurally validated, but the resulting PROCESS workspace contains no accepted-implementation-receipt marker. Runtime-attested Coordinator import is explicitly deferred until a real runtime attestation trust root exists; there is no signer, secret, or caller-named attestor interface in this workflow.

The existing narrow direct role-owned publication commands remain available for rationale, review, and verification compatibility. For example, the verification role invokes `verify submit --agent Verifier` against its exact assignment and snapshot. Agent/session fields remain `self-reported` metadata and do not become stronger evidence merely because they came from `CODEX_THREAD_ID` or an explicit parameter. Coordinator-side owner-token import is not part of `verify submit`.
