...
The writer assignment is attached to a Server user, not to a PAT. A personal
Runner account and a service account follow the same procedure. First resolve
the Runner identity from its own PAT, then use a separate repository-operator
credential to activate the assignment:
...
when the underlying identity has the required repository authority. Do not add
`admin:repo` to the Runner PAT. To retire the writer, repeat the `PUT` with
`{"active":false}`. PAT rotation for the same user leaves the assignment in
place, while changing the Runner identity requires a new assignment.
