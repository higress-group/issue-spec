...
reconciles five required application-owned indexes under an advisory lock: a
repository-scoped pg_bigm GIN index and a pg_jieba GIN index for Issue
title/body text, the same two GIN indexes for comment bodies, and the
...
version may retain obsolete unscoped pg_bigm v1 indexes or a change-key text
index; the server no longer reads or validates those indexes, and operators may
drop them later through their normal database change process instead of an
automatic startup deletion. The repository-scoped comment pg_bigm v2 index and
the comment pg_jieba index remain required and MUST NOT be dropped while
